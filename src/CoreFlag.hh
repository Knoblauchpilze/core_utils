...
namespace utils {
  ...
  class CoreFlag {
    public:
       ...
       *          following property: `length() >= reserved()`.
       ...
       *          assigned through this method. It is always smaller than the `length`
       *          value.
     ...
     *          input `length` allows to specify the number of individual values
     *          which are used to build any value of the flag.
     ...
     * @param length - the number of possible distinct individual values 
     *                 the flag.
     */
    CoreFlag(int length);
     ...
     * @param value - the default value to assign to this bit.
     * @return - an index representing the position of the bit in the internal bits
     *           array. This value is in the range `[0; length]`.
     */
    int
    addNamedBit(const std::string& name) const;
  ...
  using CoreFlagShPtr = std::shared_ptr<CoreFlag>;
}
 ...
 */
utils::CoreFlag
operator|(const utils::CoreFlag& lhs, const utils::CoreFlag& rhs) noexcept;
 ...
 */
utils::CoreFlag
operator&(const utils::CoreFlag& lhs, utils::const CoreFlag& rhs) noexcept;
 ...
 */
std::ostream&
operator<<(const utils::CoreFlag& f, std::ostream& out) noexcept;
...
std::ostream&
operator<<(std::ostream& out, const utils::CoreFlag& f) noexcept;
