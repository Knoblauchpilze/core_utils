...
namespace utils {
    ...
    onJobsCompleted()
  {
    ...
    createThreadPool(size == 0u ? getThreadPoolSize() : size);
  }

  void
  ThreadPool::notifyJobs() {
  ...
  void
  ThreadPool::enqueueJobs(const std::vector<AsynchronousJobShPtr>& jobs,
                          bool invalidate)
  {
    // Protect from concurrent accesses.
  ...
  void
  ThreadPool::createThreadPool(unsigned size) {
    ...
    }
    {
      ...
      m_resultsHandlingThread = std::thread(
        &ThreadPool::resultsHandlingLoop,
        this
      );
    }
